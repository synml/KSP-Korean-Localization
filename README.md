...
> CKAN 등록 준비 중입니다. 등록 전까지는 아래 자동/수동 설치를 이용해 주세요.
...
> - KSP가 `Program Files` 안에 있으면 권한이 필요합니다. 배치파일이 관리자 권한으로
>   다시 실행할지 물어봅니다.
> - 설치 위치를 못 찾으면 `KSP_x64.exe`가 있는 폴더를 직접 입력(또는 창에 끌어다 놓기)하면 됩니다.
